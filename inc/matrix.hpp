...
namespace mt::math
{
		...
		std::ostream& operator<<(std::ostream& out, const Matrix<T, N, M>& mat)
		{
			...
			for (int i = 0; i < mat.m_n; i++) {
				for (int j = 0; j < mat.m_m; j++)
					out << mat.m_mat[i][j] << " "; //вывод матрицы
				out << std::endl;
			}
			return out;
		}
		...
		using Vec2d = Matrix<double, 2, 1>;	// хотелось бы знать, что написано во всех этих комментариях, но видимо не судьба
		using Mat22i = Matrix<int, 2, 2>;   // выделяем типы матриц, которые часто будем использовать 
		using Mat22d = Matrix<double, 2, 2>;// (а именно двумерные и трёхмерные векторы, матрицы 2x2 и матрица 3x3)
